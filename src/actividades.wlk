...
class EquiposDeportivos inherits Actividades{
	...
	override method evaluacion(){
		return _puntosParaEvaluacion += (5*_campeonatosObtenidos) 
										+ self.puntosPorPlantel() 
										+ self.puntosPorCapitan() 
										- self.puntosPorSancion()
	}
	method puntosPorPlantel(){
		return 2 * _plantel.size()
	}
	method puntosPorCapitan(){
		return if (_capitan.esEstrella()){5} else {0}
	}
...
class ActividadesSociales inherits Actividades{
	...
	override method evaluacion(){
		return if(_sanciones > 0) {0} else {_valorParaEvaluacion}
	}
}
