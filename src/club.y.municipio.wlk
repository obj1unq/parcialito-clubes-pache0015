...
class Club{
	...
	method sancionarClub(sancion){
		if (_socios.size() >= 500){_sanciones+=sancion}
	}
	...
	method sumaDeEvaluacionDeActividades(){
		return (_actividadesSociales.sum({actividad => actividad.evaluacion()}))
				+ (_actividadesDeportivas.sum({actividad => actividad.evaluacion()}))
	}
	...
	method esPrestigioso(){
		return (self.tieneEquipoExperimentado(_actividadesDeportivas)) 
				or (self.participantesEstrellaEnActividades(_actividadesSociales))
	}
	...
	method extraerJugador(jugador, equipoDestino){
		...
		} else {self.error("No se puede transferir este jugador")}
	}
